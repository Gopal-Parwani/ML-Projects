...
def lstm_input(df ,seq_len = 1000, batch_size = 517, features = 32):
    """
    Convert the raw dataframe into LSTM input of shape (num_sequences, seq_len, features)
    Returns a torch.Tensor of shape (num_sequences, seq_len, features)
    """
    all_seqs = []
    seq_ixs = df['seq_ix'].unique()
    for seq_ix in seq_ixs:
        seq_df = df[df['seq_ix'] == seq_ix]
        seq_features = seq_df.drop(columns=['seq_ix', 'step_in_seq', 'need_prediction']).values
        seq_tensor = torch.tensor(seq_features, dtype=torch.float32)
        all_seqs.append(seq_tensor)
    
    return torch.stack(all_seqs)  # return tensor shape (batch_size, seq_len, features)
